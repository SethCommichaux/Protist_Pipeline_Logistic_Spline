...
# create dictionaries for ncbi taxonomy nodes and names files
# names = map name to taxid
...
for h,i in enumerate(os.listdir(args.p)):
	...
	if path.endswith('.fasta'):
		msa = path+'.msa'
		if os.path.exists(msa) == False: os.system('cp '+path+' '+msa)
		elif os.path.getsize(msa) == 0: os.system('cp '+path+' '+msa)
		marker_gene_group_label = i.replace('.fasta','')
		taxa = ' '.join(marker_gene_group_label.split('_')[1:])
		na = names.get(taxa,0)
		no = nodes.get(na,0)
		taxa_rank = {'family':'F','genus':'G','species':'S'}.get(no,0)
